"""
SCRIPT TAKES ALL MODEL OUTPUT FROM ~/MODEL_PRECIPITATION_ANOMALY_50KM_ENA AND 
...
# Models to read in
model_filenames = glob.glob('model_precipitation_anomaly_50km_ena/*.nc')
...
for ii, file in enumerate(model_filenames):
    if ii == 4:
        ...
        models.append(xr.open_dataset(file, decode_times=False)['pr'].fillna(-99999))
    else:
        models.append(xr.open_dataset(file, decode_times=False)['pr'])
...
proxy = pd.read_csv('proxy_data/annp_anomalies.csv')

# Changing units to mm/day from mm/year by dividing by 365.25 days
proxy['mean'] = proxy['mean']/365.25
proxy['sd'] = proxy['sd']/365.25
...
# Skill calculation in parallel on 12 cores, with 10000 replicates - 20 minutes
...
# Skill calculation in parallel on 12 cores
# Takes ~20 minutes
...
pd.concat([skill, skill_mean_baseline]).to_csv('skill_scores/pr_skill.csv', index=False)
