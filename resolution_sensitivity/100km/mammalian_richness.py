...
SCRIPT TAKES IN THE LAND MAMMAL RANGE MAPS FROM THE IUCN RED LIST, RASTERIZES
...
INPUT:
    ...
    * SPECIES_DATA/MAMMALS_TERRESTRIAL_ONLY/MAMMALS_TERRESTRIAL_ONLY.SHP
...
mamms = gpd.read_file('species_data/MAMMALS_TERRESTRIAL_ONLY/MAMMALS_TERRESTRIAL_ONLY.shp')
...
mamms['id_no'] = np.arange(1, mamms.shape[0]+1, 1)

# Transforming to albers equal area projection
mamms = mamms.to_crs('+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs')
...
mamms_array_split = np.array_split(mamms, 10)
mamms_grid_list = list()

for chunk in mamms_array_split:
    ...
    mamms_grid_list.append(geo_grid)
...
richness.rio.to_raster('resolution_sensitivity/100km/richness/mamms_richness_epsg_102008.tif')
