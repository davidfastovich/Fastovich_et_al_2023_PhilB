...
# * RESOLUTION_SENSITIVITY/100KM/SKILL_SCORES/SAR_DIAG.CSV
# 
# OUTPUT:
#   FIGURE 7:
#     * RESOLUTION_SENSITIVITY/100KM/FIGURES/SYNTHESIS_AMOC_SAR.PDF
#     
...
loglik <- read_csv("resolution_sensitivity/100km/skill_scores/sar_diag.csv") %>%
  select(logLik_fit_100, climate_simulation, species) %>% 
  rename(loglik = logLik_fit_100,  model = climate_simulation)
...
ggsave(amoc_plot, filename = "resolution_sensitivity/100km/figures/synthesis_amoc_sar.pdf", height = 8, width = 4.5, dpi = 300)
ggsave(amoc_plot, filename = "resolution_sensitivity/100km/figures/synthesis_amoc_sar.png", height = 8, width = 4.5, dpi = 300)
