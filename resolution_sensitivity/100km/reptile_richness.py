...
SCRIPT TAKES IN THE REPTILE RANGE MAPS FROM THE IUCN RED LIST, RASTERIZES
...
INPUT:
    ...
    * SPECIES_DATA/REPTILES/REPTILES.SHP
...
reps = gpd.read_file('species_data/REPTILES/REPTILES.shp')
...
reps['id_no'] = np.arange(1, reps.shape[0]+1, 1)

# Transforming to albers equal area projection
reps = reps.to_crs('+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs')
...
reps_array_split = np.array_split(reps, 10)
reps_grid_list = list()

for chunk in reps_array_split:
    ...
    reps_grid_list.append(geo_grid)
...
richness.rio.to_raster('resolution_sensitivity/100km/richness/reps_richness_epsg_102008.tif')
