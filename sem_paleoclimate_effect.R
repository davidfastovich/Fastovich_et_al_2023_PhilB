...
#   DIAGNOSTICS
#     * DIAGNOSTICS/MORAN/*
#     * DIAGNOSTICS/MORAN/VARIOGRAM/*
#     * DIAGNOSTICS/MORAN/CORRELOGRAM/*
# 
#   TABLE 1 DATA
#     * SKILL_SCORES/SAR_DIAG.CSV
# 
#   FIGURE 2
#     * FIGURES/SAR_BIODIVERSITY_COMBINED_MODEL_ENA_TWO_MODELS.PDF
# 
#   FIGURE S8
...
for(i in seq(biodiversity_files)) {
  ...
  for (j in seq(tas_files)) {
    ...
    # identical and comparable as we do later.
    fit_50_null <- errorsarlm(richness ~  1, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + .0000001))
    fit_50_no_modern_tave <- errorsarlm(richness ~  modern_pr + model_tave + model_pr + model_pr2, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + 0.0000001))
    fit_50_no_modern_pr <- errorsarlm(richness ~  modern_tave + model_tave + model_pr + model_pr2, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + 0.0000001))
    fit_50_no_paleo_tave <- errorsarlm(richness ~  modern_tave + modern_pr + model_pr + model_pr2, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + 0.0000001))
    fit_50_no_paleo_pr <- errorsarlm(richness ~  modern_tave + modern_pr + model_tave + model_pr2, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + 0.0000001))
    fit_50_no_paleo_pr2 <- errorsarlm(richness ~  modern_tave + modern_pr + model_tave + model_pr, data = model_input, listw = W_50, zero.policy=TRUE, interval = c(fit_50$lambda - 0.0000001, fit_50$lambda + 0.0000001))
    ...
    res_raster_fit_50 <- rasterFromXYZ(
      data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_50)
    ...
    # Plot and save residuals plot and Moran's I
    pdf(
      paste0(
        "diagnostics/moran/",
        paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j])),
        "_SAR.pdf"
      ),
      width = 14,
      height = 14,
    ...
    plot(
      res_raster_fit_null,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_null"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_null))
    )
    plot(
      res_raster_fit_50,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_50km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_50))
    )
    plot(
      res_raster_fit_100,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_100km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_100))
    )
    plot(
      res_raster_fit_200,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_200km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_200))
    )
    plot(
      res_raster_fit_400,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_400km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_400))
    )
    plot(
      res_raster_fit_600,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_600km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_600))
    )
    plot(
      res_raster_fit_800,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_800km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_800))
    )
    plot(
      res_raster_fit_1000,
      main = paste0(str_sub(biodiversity_files[i], 0, 4), "_", gsub("_tas_anom_50km_ena.nc", "", tas_files[j]), "_1000km"),
      xlab = paste0("Morans' I: ", raster::Moran(res_raster_fit_1000))
    )
    dev.off()

    # Plot and save correlogram of residuals - resampling for significance takes
    # really really long so I'm going to parallelize it, in an easy way. Not
    # doing it for modern because I can do that separately in a script and don't
    # want to waste the computation time
    correl_df_list <- list(
      correl_fit_fit_null = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_null)
      ),
      correl_fit_fit_50 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_50)
      ),
      correl_fit_fit_100 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_100)
      ),
      correl_fit_fit_200 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_200)
      ),
      correl_fit_fit_400 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_400)
      ),
      correl_fit_fit_600 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_600)
      ),
      correl_fit_fit_800 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_800)
      ),
      correl_fit_fit_1000 = data.frame(
        x = model_input$x,
        y = model_input$y,
        z = residuals(fit_1000)
      )
    ...
    correl_results <- mclapply(
      correl_df_list,
      function (x) correlog(x = x$x, y = x$y, z = x$z, increment = 100*1000, resamp = 100),
      mc.cores = 8
    )
    ...
    plot(
      correl_results[[1]],
      main = paste0(
        str_sub(biodiversity_files[i], 0, 4),
        "_",
        gsub("_tas_anom_50km_ena.nc", "", tas_files[j]),
        "_null"),
      ylim = c(-1, 1)
    )
    plot(
    ...
    # Plot and save variogram of residuals
    variogram_fit_null <- Variogram(res_raster_fit_null, lag = 50*1000)
    variogram_fit_50 <- Variogram(res_raster_fit_50, lag = 50*1000)
    variogram_fit_100 <- Variogram(res_raster_fit_100, lag = 50*1000)
    variogram_fit_200 <- Variogram(res_raster_fit_200, lag = 50*1000)
    variogram_fit_400 <- Variogram(res_raster_fit_400, lag = 50*1000)
    variogram_fit_600 <- Variogram(res_raster_fit_600, lag = 50*1000)
    variogram_fit_800 <- Variogram(res_raster_fit_800, lag = 50*1000)
    variogram_fit_1000 <- Variogram(res_raster_fit_1000, lag = 50*1000)
    ...
    lines(
      x = variogram_fit_null@variogram$distance,
      y = variogram_fit_null@variogram$gamma
    )
    plot(
      variogram_fit_50,
      main = paste0(
    ...
    lines(
      x = variogram_fit_50@variogram$distance,
      y = variogram_fit_50@variogram$gamma
    )
    plot(
      variogram_fit_100,
      main = paste0(
    ...
    lines(
      x = variogram_fit_100@variogram$distance,
      y = variogram_fit_100@variogram$gamma
    )
    plot(
      variogram_fit_200,
      main = paste0(
    ...
    lines(
      x = variogram_fit_200@variogram$distance,
      y = variogram_fit_200@variogram$gamma
    )
    plot(
      variogram_fit_400,
      main = paste0(
    ...
    lines(
      x = variogram_fit_400@variogram$distance,
      y = variogram_fit_400@variogram$gamma
    )
    plot(
      variogram_fit_600,
      main = paste0(
    ...
    lines(
      x = variogram_fit_600@variogram$distance,
      y = variogram_fit_600@variogram$gamma
    )
    plot(
      variogram_fit_800,
      main = paste0(
    ...
    lines(
      x = variogram_fit_800@variogram$distance,
      y = variogram_fit_800@variogram$gamma
    )
    plot(
      variogram_fit_1000,
      main = paste0(
    ...
    lines(
      x = variogram_fit_1000@variogram$distance,
      y = variogram_fit_1000@variogram$gamma
    )
    dev.off()
    ...
    if(i == 2) {
      ...
      fit <- fit_50
      fit_null <- fit_50_null
      fit_no_paleo_tave <- fit_50_no_paleo_tave
      fit_no_paleo_pr <- fit_50_no_paleo_pr
      fit_no_paleo_pr2 <- fit_50_no_paleo_pr2
      fit_no_modern_tave <- fit_50_no_modern_tave
      fit_no_modern_pr <- fit_50_no_modern_pr
      W <- W_50 
    }
    ...
    diag_list <- c(
      ...
      list(
        tibble(
          ...
          coef_paleo_tave = coef(fit)["model_tave"],
          coef_paleo_pr = coef(fit)["model_pr"],
          coef_paleo_pr2 = coef(fit)["model_pr2"],
          coef_modern_tave = coef(fit)["modern_tave"],
          coef_modern_pr = coef(fit)["modern_pr"],
          AIC_fit_null = AIC(fit_null),
          AIC_fit_50 = AIC(fit_50),
          ...
          pseudo_r2full_fit_50 = summary(fit_50, Nagelkerke = TRUE)$NK,
          ...
          pseudo_r2env_fit_50 = abs(summary(fit_50, Nagelkerke = TRUE)$NK - summary(fit_null, Nagelkerke = TRUE)$NK),
          ...
          moran_fit_50 = raster::Moran(res_raster_fit_50),
          ...
          modern_tave_sig = summary(fit)$Coef["modern_tave","Pr(>|z|)"] <= 0.05,
          modern_pr_sig = summary(fit)$Coef["modern_pr","Pr(>|z|)"] <= 0.05,
          paleo_tave_sig = summary(fit)$Coef["model_tave","Pr(>|z|)"] <= 0.05,
          paleo_pr_sig = summary(fit)$Coef["model_pr","Pr(>|z|)"] <= 0.05,
          paleo_pr2_sig = summary(fit)$Coef["model_pr2","Pr(>|z|)"] <= 0.05,
          climate_simulation = gsub("_tas_anom_50km_ena.nc", "", tas_files[j]),
    ...
    if(model_summary_fit$Coef["model_pr","Pr(>|z|)"] <= 0.05) {
...
# paleo_tave is significant 39 times
# paleo_pr2 is significant 11 times
# paleo_pr is significant 25 times
# any precipitation variable is significant 33 times
diag_list %>%
  bind_rows() %>% 
  write_csv("skill_scores/sar_diag.csv")
...
# Reordering the facet wrap plots from alphabetical to model skill
...
model_tave_raster_list[[1]] <- add_column(model_tave_raster_list[[1]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[1]))
model_tave_raster_list[[2]] <- add_column(model_tave_raster_list[[2]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[2]))
model_tave_raster_list[[3]] <- add_column(model_tave_raster_list[[3]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[3]))
model_tave_raster_list[[4]] <- add_column(model_tave_raster_list[[4]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[4]))
model_tave_raster_list[[5]] <- add_column(model_tave_raster_list[[5]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[5]))
model_tave_raster_list[[6]] <- add_column(model_tave_raster_list[[6]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[6]))
model_tave_raster_list[[7]] <- add_column(model_tave_raster_list[[7]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[7]))
model_tave_raster_list[[8]] <- add_column(model_tave_raster_list[[8]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[8]))
model_tave_raster_list[[9]] <- add_column(model_tave_raster_list[[9]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[9]))
model_tave_raster_list[[10]] <- add_column(model_tave_raster_list[[10]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[10]))
model_tave_raster_list[[11]] <- add_column(model_tave_raster_list[[11]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[11]))
model_tave_raster_list[[12]] <- add_column(model_tave_raster_list[[12]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[12]))
model_tave_raster_list[[13]] <- add_column(model_tave_raster_list[[13]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[13]))
...
# Add model identifier to each tibble with the list
model_pr_raster_list[[1]] <- add_column(model_pr_raster_list[[1]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[1]))
model_pr_raster_list[[2]] <- add_column(model_pr_raster_list[[2]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[2]))
model_pr_raster_list[[3]] <- add_column(model_pr_raster_list[[3]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[3]))
model_pr_raster_list[[4]] <- add_column(model_pr_raster_list[[4]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[4]))
model_pr_raster_list[[5]] <- add_column(model_pr_raster_list[[5]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[5]))
model_pr_raster_list[[6]] <- add_column(model_pr_raster_list[[6]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[6]))
model_pr_raster_list[[7]] <- add_column(model_pr_raster_list[[7]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[7]))
model_pr_raster_list[[8]] <- add_column(model_pr_raster_list[[8]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[8]))
model_pr_raster_list[[9]] <- add_column(model_pr_raster_list[[9]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[9]))
model_pr_raster_list[[10]] <- add_column(model_pr_raster_list[[10]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[10]))
model_pr_raster_list[[11]] <- add_column(model_pr_raster_list[[11]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[11]))
model_pr_raster_list[[12]] <- add_column(model_pr_raster_list[[12]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[12]))
model_pr_raster_list[[13]] <- add_column(model_pr_raster_list[[13]], model = gsub("_tas_anom_50km_ena.nc", "", tas_files[13]))
...
# PLOT 2A,C,E,G - TEMPERATURE FOR SELECT MODELS --------------------------------

# Creating an annotation data frame to label facets
ann_df <- tave_fitted_tibble %>% 
  filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")) %>%
  select(model) %>%
  distinct() %>% 
  add_column(label = LETTERS[c(5, 7, 1, 3)], sig = NA, species = NA)

biodiversity_temperature_models <- tave_fitted_tibble %>% 
  filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")) %>% 
  ggplot(aes(x = model_tave, y = fit, color = species, linetype = sig)) + 
  geom_point(data = model_tave_richness_df %>% filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")), aes(x = model_tave, y = richness, color = species), alpha = 0.025) +
  geom_path() + 
  ...
  geom_text(data = ann_df, aes(x = -Inf, y = -Inf, label = label), color = "black", size = 4, hjust = -0.25, vjust = -15.8, fontface = "bold") +
  ylab("Species Richness") + 
  xlab("Temperature Anomaly (K)") + 
  theme(panel.spacing.y = unit(-.2, "cm"),
        strip.text = element_text(color = "black", vjust = -0.05),
        legend.position="none")

# PLOT 2B,D,F,H - PRECIPITATION FOR SELECT MODELS ------------------------------

ann_df <- pr_fitted_tibble %>% 
  filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")) %>%
  select(model) %>%
  distinct() %>% 
  add_column(label = LETTERS[c(6, 8, 2, 4)], sig = NA, species = NA)

biodiversity_precipitation_models <- pr_fitted_tibble %>% 
  filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")) %>% 
  ggplot(aes(x = model_pr, y = fit, color = species, linetype = sig)) + 
  geom_point(data = model_pr_richness_df %>% filter(model %in% c("Proxy Kriging", "TraCE-21ka\n(Statistcally Downscaled)", "IPSL", "MIROC-W")), aes(x = model_pr, y = richness, color = species), alpha = 0.025) +
  geom_path() + 
  scale_color_brewer(palette = "Dark2", name = "Species") + 
  scale_fill_brewer(palette = "Dark2", name = "Species") + 
...
# PLOT 2 - PRECIPITATION AND TEMPERATURE COMBINED FOR SELECT MODELS ------------
# Combine Plot 1 and Plot 2 into a single plot using patchwork and then save
biodiversity_models_combined <- biodiversity_temperature_models + biodiversity_precipitation_models

ggsave(biodiversity_models_combined, filename = "figures/sar_biodiversity_combined_model_ena_two_models_quadratic.pdf", height = 10, width = 7, dpi = 300)
ggsave(biodiversity_models_combined, filename = "figures/sar_biodiversity_combined_model_ena_two_models_quadratic.png", height = 10, width = 7, dpi = 300)

# PLOT S2 - TEMPERATURE FOR ALL MODELS PALEOCLIMATE + MODERN ONLY --------------
...
ggsave(supp_modern_biodiversity_temperature_models, filename = "figures/sar_biodiversity_temperature_model_ena_modern_correlates_quadratic.pdf", height = 10, width = 8, dpi = 300)
ggsave(supp_modern_biodiversity_temperature_models, filename = "figures/sar_biodiversity_temperature_model_ena_modern_correlates_quadratic.png", height = 10, width = 8, dpi = 300)

# PLOT S3 - PRECIPITATION FOR ALL MODELS PALEOCLIMATE + MODERN ONLY ------------
...
ggsave(supp_modern_biodiversity_precipitation_models, filename = "figures/sar_biodiversity_precipitation_model_ena_modern_correlates_quadratic.pdf", height = 10, width = 8, dpi = 300)
ggsave(supp_modern_biodiversity_precipitation_models, filename = "figures/sar_biodiversity_precipitation_model_ena_modern_correlates_quadratic.png", height = 10, width = 8, dpi = 300)
