...
OUTPUT:
    PRECIPITATION
        * MODEL_PRECIPITATION_ANOMALY_50KM_ENA/PROXY_KRIGING_PR_ANOM_50KM_ENA.NC
    
    TEMPERATURE
        * MODEL_TEMPERATURE_ANOMALY_50KM_ENA/PROXY_KRIGING_TAS_ANOM_50KM_ENA.NC
...
# Create geodataframe
proxy_tas_gdf = geopandas.GeoDataFrame(
    proxy_tas,
...
# Datum: NAD83
proxy_tas_gdf = proxy_tas_gdf.to_crs(
...
#############################################
# READ IN PROXY PRECIPITATION RECONSTRUCTIONS
#############################################
...
# Using geopandas to make quick work of reprojection
# Create geodataframe
proxy_pr_gdf = geopandas.GeoDataFrame(
    proxy_pr,
...
# Standard parallels = 20 N, 60 N
# Datum: NAD83
proxy_pr_gdf = proxy_pr_gdf.to_crs(
    '+proj=aea +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs'
    )
...
Y = np.arange(-3000000, 3000000, 50000).astype('float32')
X = np.arange(-4000000, 4000000, 50000).astype('float32')
...
# Create an xarray instance from the kriged array and write to netCDF
tas_ds = xr.Dataset(
    data_vars=dict(
...
tas_ds.to_netcdf('model_temperature_anomaly_50km_ena/PROXY_KRIGING_tas_anom_50km_ena.nc')
...
pr_ds.to_netcdf('model_precipitation_anomaly_50km_ena/PROXY_KRIGING_pr_anom_50km_ena.nc')